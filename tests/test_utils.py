...
def test_make_search_docs_url():   
    dataset_id = "test_id"
    assert f"https://search.axds.co/v2/docs?verbose=false&id={dataset_id}" == utils.make_search_docs_url(dataset_id)


def test_make_metadata_url():   
    filter = "filter"
    assert f"{utils.baseurl}/metadata/filter/custom?filter={filter}" == utils.make_metadata_url(filter)


def test_make_filter():
    internal_id = "internal_id"
    assert "parameterGroups" in utils.make_filter(internal_id=internal_id, parameterGroupId=10)
    assert "parameterGroups" not in utils.make_filter(internal_id=internal_id, parameterGroupId=None)
    ...
    results = {"uuid": "uuid",
               "label": "label",
                ...
                "source": {"meta": {"attributes": {"institution": "institution",
                                    ...
                                    "variables": {"variable_name": {"attributes": {"standard_name": "standard_name",
                                                                    "units": "units",
                                                                    "unit_id": "unit_id",
                                                                    "long_name": "long_name",
